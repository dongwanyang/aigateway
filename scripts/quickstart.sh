...
trap 'rm -f "$tmp_state"' EXIT
{
  ...
  echo "AIGATEWAY_INSTALL_STATE_VERSION=3"
...
GPU_COMPOSE_FILE="$RUNTIME_DIR/docker-compose.gpu.generated.yml"
if [[ "$accelerator" == "cuda" && "$needs_studio" == "true" \
      && "$comfyui_mode" == "container" ]]; then
  python3 "$ROOT_DIR/scripts/render-gpu-topology.py" \
    --output-compose "$GPU_COMPOSE_FILE" \
