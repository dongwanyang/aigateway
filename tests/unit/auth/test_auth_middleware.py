...
- authenticate / authenticate_admin: mocked key_store flows
"""
import os
import sys
...
class TestAuthenticateMiddleware:
    """Test authenticate with mocked key_store."""
    ...
    async def test_authenticate_valid_key(self):
        ...
        result = await authenticate(
            MagicMock(app=MagicMock(state=MagicMock(key_store=key_store))),
            api_key=None,
    ...
    async def test_authenticate_missing_key_raises_401(self):
        ...
        assert exc_info.value.detail["error"]["message"] == "API key required"
        assert exc_info.value.headers == {"WWW-Authenticate": "Bearer"}
        key_store.validate.assert_not_called()
    ...
    async def test_authenticate_revoked_key_raises_403(self):
        ...
        assert exc_info.value.detail["error"]["message"] == "API key has been revoked"
    ...
    async def test_authenticate_suspended_key_raises_403(self):
        ...
        assert exc_info.value.status_code == 403
        assert exc_info.value.detail["error"]["code"] == "forbidden"
        assert exc_info.value.detail["error"]["message"] == "API key is suspended"
