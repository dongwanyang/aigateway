...
class _ASGISyncClient:
    """Small sync facade over HTTPX's ASGI transport for sync route tests.

    Starlette's threaded TestClient can deadlock when a route owns background
    asyncio tasks.  Running the request on an explicit loop also makes leaked
    tasks visible at loop shutdown.
    """
...
def _make_client(monkeypatch: pytest.MonkeyPatch) -> tuple[_ASGISyncClient, FastAPI]:
    ...
    # 路由层测试验证持久化和派生参数，不应遗留后台 Task。真实 strong-ref
    # 生命周期由下面的 test_spawn_import_task_* 单独覆盖。
...
def test_spawn_import_task_tracks_task_in_app_state() -> None:
    ...
    active, task = _asyncio.run(_drive())
    assert task.done()
...
def test_run_code_import_task_passes_real_symbol_name_to_graph_lookup(
    ...
    monkeypatch.setattr(routes_mod, "_write_task_state", AsyncMock())
    monkeypatch.setattr(routes_mod, "_delete_task_key", AsyncMock())
    monkeypatch.setattr(routes_mod, "_append_repository", AsyncMock())

    async def _run_inline(_loop, _executor, func, *args):
        return func(*args)

    monkeypatch.setattr(asyncio.BaseEventLoop, "run_in_executor", _run_inline)
    ...
    async def _drive() -> None:
        await asyncio.wait_for(
            routes_mod._run_code_import_task(
                app_state=app_state,
                task_id="t-sym",
...
def test_run_code_import_task_batches_encode_calls_at_64_boundary(
    ...
    monkeypatch.setattr(routes_mod, "_write_task_state", write_state)
    monkeypatch.setattr(routes_mod, "_delete_task_key", AsyncMock())
    monkeypatch.setattr(routes_mod, "_append_repository", AsyncMock())
    ...
    # 编排层必须明确写入 completed；SQLite 持久化由独立 store 测试覆盖。
    assert any(
        await_call.args[2].get("status") == "completed"
        for await_call in write_state.await_args_list
    )
...
def test_splitting_progress_written_to_sqlite(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(_aio.BaseEventLoop, "run_in_executor", _run_inline)
    monkeypatch.setattr(splitter_mod, "build_symbol_chunks", fake_build)
    monkeypatch.setattr(graph_builder_mod, "build_code_graph", lambda src, dst: dst)

    async def drive():
        await routes._run_code_import_task(
        ...
        # progress_cb uses run_coroutine_threadsafe and is deliberately
        # fire-and-forget; yield once so both scheduled updates are persisted.
        await _aio.sleep(0)
