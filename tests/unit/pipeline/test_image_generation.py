...
@pytest.mark.asyncio
async def test_image_429_rate_limit():
...
@pytest.mark.asyncio
async def test_image_timeout_error():
...
@pytest.mark.asyncio
async def test_image_b64_json_response():
    ...
    async def fake_post(url, headers, json):
        resp = MagicMock()
        resp.status_code = 200
        resp.json.return_value = {"created": 1, "data": [{"b64_json": "SGVsbG8gV29ybGQh"}]}
        resp.raise_for_status = MagicMock()
        return resp
    ...
    msg = result["choices"][0]["message"]
    assert msg["role"] == "assistant"
    assert "SGVsbG8gV29ybGQh" in msg["content"]
    assert result["choices"][0]["finish_reason"] == "stop"
    assert "usage" in result
