...
def test_build_request_preserves_original_reference_image_after_captioning():
    plugin = _make_plugin()
    ctx = PipelineContext(
        request={
            "messages": [
                {"role": "user", "content": "a caption generated by MOL"}
            ],
            "reference_image_urls": ["data:image/png;base64,cG5n"],
        },
    ...
    assert request.reference_images[0].source_url == (
