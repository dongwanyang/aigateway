...
@pytest.mark.asyncio
async def test_middleware_generates_trace_id_when_absent():
    app = _make_app()
...
@pytest.mark.asyncio
async def test_middleware_uses_incoming_x_trace_id():
    app = _make_app()
    async with AsyncClient(
        transport=ASGITransport(app=app),
        base_url="http://testserver",
        ...
        resp = await client.get(
            "/ping",
            headers={"x-trace-id": "incoming-id-123"},
        )
...
@pytest.mark.asyncio
async def test_middleware_flushes_to_redis():
