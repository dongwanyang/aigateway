...
class TestBuildModelList:
    ...
    def test_malformed_provider_entries_are_ignored_or_safely_defaulted(self):
        config = _make_config(
            capabilities={"configured": 77, "bad-cap": 66},
            modalities={
                "configured": ["llm"],
                "empty-modality": ["mllm"],
                "bad-modality": ["llm"],
        ...
        providers = {
            ...
            "bad-groups": {"model_grouper": "invalid"},
            "p": {
                "model_grouper": [
                    "invalid",
                    {"models": "invalid"},
        ...
        models = ModelRouterStrategy(config, providers).get_model_list()
        ...
        assert by_name["configured"].modality == ["llm"]
        assert by_name["empty-modality"].modality == ["mllm"]
        assert by_name["empty-modality"].capability_score == 80
        assert by_name["bad-modality"].modality == ["llm"]
        assert by_name["bad-modality"].capability_score == 50
        assert by_name["bad-price"].fallback_models == []

    def test_invalid_prompt_price_and_non_mapping_price_default_to_zero(self):
        config = _make_config()
        providers = {
            "p": {
                "model_grouper": [
        ...
        models = ModelRouterStrategy(config, providers).get_model_list()
        assert [model.price_per_request for model in models] == [0.0, 0.0]
...
class TestRoutingHint:
    ...
    @pytest.mark.parametrize("hint", ["best quality", "cheapest"])
    def test_named_hint_without_matching_modality_falls_back(self, hint):
        strategy = _make_strategy()
        decision = asyncio.run(
            strategy.route(
                complexity_score=50,
    ...
    def test_hint_for_unavailable_specific_model_falls_back(self):
        strategy = _make_strategy()
        strategy.update_model_availability("model-c", False)
        decision = asyncio.run(
            strategy.route(
...
class TestFallback:
    ...
    def test_unavailable_override_without_any_fallback_raises(self):
        only = ModelConfig(
            name="only",
            provider="p",
            modality=["llm"],
        ...
        strategy = _make_strategy(default_model="only", providers={})
        with pytest.raises(ModelRoutingError, match="无可用降级模型"):
            asyncio.run(
                strategy.route(
                    complexity_score=10,
    ...
    def test_try_fallback_uses_cross_provider_then_same_provider(self):
        strategy = _make_strategy(providers={})
        target = ModelConfig("target", "a", ["llm"], 90, 1.0, ["missing"], False)
        cross = ModelConfig("cross", "b", ["llm"], 70, 0.5)
        same = ModelConfig("same", "a", ["llm"], 80, 0.6)
        assert strategy._try_fallback(target, [target, same, cross]) is cross
        assert strategy._try_fallback(target, [target, same]) is same
        same.is_available = False
        assert strategy._try_fallback(target, [target, same]) is None

    def test_default_decision_reports_unknown_provider_when_default_unregistered(self):
        strategy = _make_strategy(default_model="missing", providers={})
        decision = strategy._default_decision(42)
        assert decision.selected_model == "missing"
        assert decision.selected_provider == "unknown"
