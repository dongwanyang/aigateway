...
class TestL2Cache:
    ...
    async def test_l2_search_store_with_meta(self):
        ...
        meta = {
            ...
            "pipeline_kind": "understanding",
            "pipeline_version": "2:routing-2:coding",
            "model_family": "gpt-4o",
            "cache_scope": "group",
            "scope_id": "user1",
        }
        with patch("aigateway_core.prefix.cache.l2_search.store") as mock_store:
            ...
            assert (
                mock_store.call_args.kwargs["pipeline_version"]
                == "2:routing-2:coding"
            )
...
class TestL3Cache:
    ...
    @pytest.mark.asyncio
    async def test_l3_query_rejects_different_pipeline_version(self):
        self.mock_qdrant.query_vector = AsyncMock(
            return_value={
                "id": "vec-old",
                "score": 0.99,
        ...
        result = await self.cm.l3_query(
            [0.1] * 1024,
            pipeline_version="2:routing-2:coding",
        )
        assert result is None
        assert self.mock_qdrant.query_vector.call_args.kwargs[
            "payload_filters"
        ] == {"pipeline_version": "2:routing-2:coding"}
