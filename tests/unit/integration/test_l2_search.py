...
class TestStore:
    ...
    async def test_none_client(self):
        """None client 直接返回。"""
        await store(
            None, "key", "value", "prompt", "kind", "1", "model", "scope", "user"
        )
        # 无异常即通过
    ...
    async def test_empty_prompt(self):
        ...
        await store(
            client, "key", "value", "", "kind", "1", "model", "scope", "user"
        )
    ...
    async def test_successful_write(self):
        ...
        await store(
            ...
            normalized_prompt, "understanding", "1", "gpt-4o", "group", "user1"
        )
    ...
    async def test_value_truncated(self):
        ...
        long_value = "x" * 20000
        await store(client, "key", long_value, "prompt", "k", "1", "m", "s", "u")
    ...
    async def test_exception_logged(self):
        ...
        await store(client, "key", "value", "prompt", "k", "1", "m", "s", "u")
        # 无异常即通过
...
class TestSearch:
    ...
    async def test_empty_prompt(self):
        """空 prompt 返回 None。"""
        client = MagicMock()
        result = await search(client, "", "k", "1", "m", "s", "u")
        assert result is None
    ...
    async def test_no_results(self):
        ...
        result = await search(
            client, "unknown prompt", "k", "1", "m", "s", "u"
        )
        assert result is None
    ...
    async def test_successful_hit(self):
        ...
        result = await search(
            client, "similar prompt", "k", "1", "m", "s", "u"
        )
