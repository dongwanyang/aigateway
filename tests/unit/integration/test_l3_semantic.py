...
class TestComputeL3Vector:
    def test_returns_none_when_torch_missing(self):
        with patch.dict(sys.modules, {"torch": None}):
            ...
            result = l3_semantic._compute_l3_vector_sync("test text")
            assert result is None

    def test_returns_none_when_transformers_missing(self):
        """缺少 transformers 时应返回 None。"""
        with patch.dict(sys.modules, {"transformers": None}):
            result = l3_semantic._compute_l3_vector_sync("test")
            assert result is None

    @pytest.mark.asyncio
    async def test_async_vector_computation_is_offloaded_to_thread(self):
        """请求事件循环不能直接运行重型 transformers 推理。"""
        run_in_thread = AsyncMock(return_value=None)
        with patch.object(l3_semantic.asyncio, "to_thread", run_in_thread):
            result = await l3_semantic._compute_l3_vector("test")
        assert result is None
        run_in_thread.assert_awaited_once_with(
            l3_semantic._compute_l3_vector_sync,
            "test",
            True,
        )
