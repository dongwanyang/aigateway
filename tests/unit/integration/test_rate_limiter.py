...
class TestDispatch:
    ...
    async def test_exempt_health_not_rate_limited(self):
        ...
        for _ in range(10):
            resp = await _request(app, "/health")
    ...
    async def test_exempt_metrics_not_rate_limited(self):
        ...
        app.add_middleware(RateLimiterMiddleware, max_requests=1)
        for _ in range(10):
            resp = await _request(app, "/metrics")
    ...
    async def test_non_protected_path_not_rate_limited(self):
        ...
        app.add_middleware(RateLimiterMiddleware, max_requests=1)
        for _ in range(10):
            resp = await _request(app, "/v1/chat/completions")
            assert resp.status_code == 200
    ...
    async def test_protected_path_rate_limited_after_threshold(self):
        ...
        resp2 = await _request(app, "/admin/test")
        assert resp2.status_code == 200
        # Third should be 429
        resp3 = await _request(app, "/admin/test")
    ...
    async def test_rate_limit_429_has_retry_after_header(self):
        ...
        await _request(app, "/admin/test")  # First call succeeds
        resp = await _request(app, "/admin/test")  # Second call should be 429
        assert resp.status_code == 429
        assert "retry-after" in resp.headers

    @pytest.mark.asyncio
    async def test_custom_protected_prefix(self):
        ...
        # First calls succeed
        await _request(app, "/admin/test")
        await _request(app, "/internal/test")
        # Second calls should be 429
        assert (await _request(app, "/admin/test")).status_code == 429
        assert (await _request(app, "/internal/test")).status_code == 429
