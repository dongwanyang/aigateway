...
这是显式的真实环境套件；缺少开关时必须失败，不能把未执行报告成成功:
...
# 此测试会切插件配置 + 打真实上游 + 消耗真实 token/配额。
# 显式启用:RUN_RAG_AB=1 python3 -m pytest tests/test_rag_token_savings_ab.py -v -s


@pytest.fixture(scope="module", autouse=True)
def require_real_rag_environment():
    assert os.environ.get("RUN_RAG_AB") == "1", (
        "真实 RAG A/B 测试未执行：必须设置 RUN_RAG_AB=1，并提供运行中的 "
        "gateway、真实 key 和 provider 配额"
    )
