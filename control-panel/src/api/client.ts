...
export async function getDraftStatus(draftId: string): Promise<{ status: string; expiresAt: number; attemptNumber: number; maxAttempts: number; progress: number; stage: string; workflowVersion: string }> { const json = await rawJson<{ status?: string; expires_at?: number; attempt_number?: number; max_attempts?: number; progress?: number; stage?: string; workflow_version?: string }>(`/admin/draft/${encodeURIComponent(draftId)}`); return { status: json.status ?? 'unknown', expiresAt: json.expires_at ?? 0, attemptNumber: json.attempt_number ?? 1, maxAttempts: json.max_attempts ?? 5, progress: json.progress ?? 0, stage: json.stage ?? json.status ?? 'unknown', workflowVersion: json.workflow_version ?? '' } }
