...
export async function confirmDraft(draftId: string): Promise<ConfirmDraftResult> {
  const json = await rawJson<{
    ...
    algorithm?: string
  }>(`/admin/draft/${encodeURIComponent(draftId)}/confirm`, { method: 'POST' })
  if (json.media_type === 'video' && json.video_id) {
    return { videoId: json.video_id, status: json.status ?? 'generating', mediaType: 'video' }
  }
...
export async function rejectDraft(
  ...
  const json = await rawJson<{
    ...
    max_attempts?: number
  }>(`/admin/draft/${encodeURIComponent(draftId)}/reject`, { method: 'POST' })
...
export async function getDraftStatus(
  ...
  const json = await rawJson<{
    ...
    attempt_number?: number
    max_attempts?: number
  }>(`/admin/draft/${encodeURIComponent(draftId)}`)
  return {
...
// ------------------------------------------------------------------
// Admin: API keys / quotas / groups
// ------------------------------------------------------------------

export async function listApiKeys(page = 1, pageSize = 20): Promise<ApiResponse<ApiKeyListData>> {
  return fetchJson<ApiKeyListData>(`/admin/api-keys?page=${page}&page_size=${pageSize}`)
}

export async function createApiKey(body: CreateApiKeyRequest): Promise<ApiResponse<CreateApiKeyData>> {
  return fetchJson<CreateApiKeyData>('/admin/api-keys', { method: 'POST', body: JSON.stringify(body) })
}

export async function deleteApiKey(keyId: string): Promise<ApiResponse<RevokedKeyData>> {
  return fetchJson<RevokedKeyData>(`/admin/api-keys/${encodeURIComponent(keyId)}`, { method: 'DELETE' })
}

export async function rotateApiKey(keyId: string): Promise<{ data: { key: string; warning: string } }> {
  return fetchJson<{ key: string; warning: string }>(`/admin/api-keys/${encodeURIComponent(keyId)}/rotate`, {
    method: 'POST',
    body: JSON.stringify({}),
  })
}
...
export async function updateApiKeyQuota(keyId: string, body: UpdateQuotaRequest): Promise<ApiResponse<UpdateQuotaData>> {
...
export async function deleteGroup(groupId: string): Promise<ApiResponse<{ deleted: boolean }>> {
  return fetchJson<{ deleted: boolean }>(`/admin/groups/${encodeURIComponent(groupId)}`, { method: 'DELETE' })
}
...
export interface MetricsQueryResponse {
  status: string
  data: { resultType: string; result: Array<{ metric?: Record<string, string>; values?: Array<{ timestamp: string; value: string }> }> }
}
...
export interface MetricsJsonData {
  ...
  keys: { total_keys: number; total_daily_tokens_used: number; total_monthly_cost_used: number; total_requests: number }
  circuit_breakers: Record<string, unknown>
  uptime_seconds: number
}

export async function getMetricsJson(): Promise<ApiResponse<MetricsJsonData>> {
  return fetchJson<MetricsJsonData>('/admin/metrics-json')
}
...
export async function getCostLedger(params?: {
  limit?: number; offset?: number; start?: number | null; end?: number | null; user_id?: string | null; group_id?: string | null; model?: string | null
}): Promise<LedgerRow[]> {
...
export async function getCostSummary(days?: number): Promise<CostSummary> {
  return rawJson<CostSummary>(`/admin/costs/summary${days ? `?days=${days}` : ''}`)
}

// ------------------------------------------------------------------
// Admin config / plugins / debug
// ------------------------------------------------------------------
...
export interface DebugConfig { dimensions?: Record<string, boolean>; [key: string]: unknown }

export async function getPluginsConfig(): Promise<ApiResponse<PluginsConfigData>> {
  return fetchJson<PluginsConfigData>('/admin/plugins-config')
}

export async function togglePlugin(name: string, enabled: boolean): Promise<ApiResponse<{ name: string; enabled: boolean }>> {
  return fetchJson<{ name: string; enabled: boolean }>('/admin/plugins-config', { method: 'PUT', body: JSON.stringify({ name, enabled }) })
}

export async function getGlobalConfig(): Promise<ApiResponse<GlobalConfigData>> {
  return fetchJson<GlobalConfigData>('/admin/global-config')
}
...
export async function updateGlobalConfig(config: { hot_reload: boolean; debug_mode?: boolean }): Promise<ApiResponse<GlobalConfigData>> {
  return fetchJson<GlobalConfigData>('/admin/global-config', { method: 'PUT', body: JSON.stringify(config) })
}
...
export async function updateFullConfig(config: Record<string, unknown>): Promise<ApiResponse<{ updated: boolean }>> {
  return fetchJson<{ updated: boolean }>('/admin/config', { method: 'PUT', body: JSON.stringify(config) })
}

export async function setPluginDebug(name: string, enabled: boolean): Promise<ApiResponse<{ name: string; enabled: boolean }>> {
  return fetchJson<{ name: string; enabled: boolean }>(`/admin/debug/plugins/${encodeURIComponent(name)}`, { method: 'PUT', body: JSON.stringify({ enabled }) })
}

export async function getDebugConfig(): Promise<DebugConfig> {
  const res = await fetchJson<DebugConfig>('/admin/debug/config')
  return res.data
}

export async function updateDebugSection(section: string, enabled: boolean): Promise<ApiResponse<DebugConfig>> {
  return fetchJson<DebugConfig>(`/admin/debug/${encodeURIComponent(section)}`, { method: 'PUT', body: JSON.stringify({ enabled }) })
}

export async function testProviderConnectivity(provider: string, config?: Record<string, unknown>): Promise<ApiResponse<Record<string, unknown>>> {
  return fetchJson<Record<string, unknown>>(`/admin/providers/${encodeURIComponent(provider)}/test`, { method: 'POST', body: JSON.stringify(config ?? {}) })
}

export async function fetchProviderModels(provider: string, config?: Record<string, unknown>): Promise<ApiResponse<{ models: string[] }>> {
  return fetchJson<{ models: string[] }>(`/admin/providers/${encodeURIComponent(provider)}/models`, { method: 'POST', body: JSON.stringify(config ?? {}) })
}
...
export async function getRequestLogs(params: { page?: number; pageSize?: number; user_id?: string; model?: string; status?: string; cache_only?: boolean }): Promise<ApiResponse<LogsData>> {
  const qs = new URLSearchParams()
  ...
  return fetchJson<LogsData>(`/admin/logs?${qs}`)
}

export async function deleteAllLogs(): Promise<ApiResponse<{ deleted: boolean }>> {
  return fetchJson<{ deleted: boolean }>('/admin/logs', { method: 'DELETE' })
}

export async function batchDeleteLogs(requestIds: string[]): Promise<ApiResponse<{ deleted: number; requested: number }>> {
  return fetchJson<{ deleted: number; requested: number }>('/admin/logs/batch-delete', { method: 'POST', body: JSON.stringify({ request_ids: requestIds }) })
}

export async function getTraceDetail(traceId: string): Promise<ApiResponse<TraceDetail>> {
  return fetchJson<TraceDetail>(`/admin/trace/${encodeURIComponent(traceId)}`)
}
...
export async function getRuntimeCapabilities(): Promise<ApiResponse<RuntimeCapabilitiesData>> {
  return fetchJson<RuntimeCapabilitiesData>('/admin/capabilities')
}
...
export async function listRagDocuments(): Promise<ApiResponse<{ documents: RagDocument[] }>> {
  return fetchJson<{ documents: RagDocument[] }>('/admin/rag/documents')
}

export async function importRagDocument(params: { url?: string; content?: string; filename?: string; chunk_strategy?: string; chunk_size?: number; chunk_overlap?: number }): Promise<ApiResponse<{ doc_id: string; filename: string; chunk_count: number; total_tokens: number; elapsed_ms: number }>> {
  return fetchJson('/admin/rag/documents', { method: 'POST', body: JSON.stringify(params) })
}

export async function deleteRagDocument(docId: string): Promise<ApiResponse<{ deleted: boolean }>> {
  return fetchJson<{ deleted: boolean }>(`/admin/rag/documents/${encodeURIComponent(docId)}`, { method: 'DELETE' })
}
...
export async function importCodeRepository(payload: FormData | CodeImportJsonPayload): Promise<{ task_id: string; status: 'pending' }> {
  const headers = await ensureAuthHeaders()
  ...
  if (!res.ok) throw new Error(await errorText(res, 'Code import failed'))
  return res.json()
}

export async function listCodeImportTasks(): Promise<CodeImportTask[]> {
  return rawJson<CodeImportTask[]>('/admin/rag/code/tasks')
}

export async function getCodeImportTask(taskId: string): Promise<CodeImportTask> {
  return rawJson<CodeImportTask>(`/admin/rag/code/tasks/${encodeURIComponent(taskId)}`)
}

export async function cancelCodeImportTask(taskId: string): Promise<{ task_id: string; status: CodeImportTaskStatus }> {
  return rawJson(`/admin/rag/code/tasks/${encodeURIComponent(taskId)}/cancel`, { method: 'POST' })
}

export async function listCodeRepositories(): Promise<CodeRepositoryImport[]> {
  return rawJson<CodeRepositoryImport[]>('/admin/rag/code/repositories')
}

export async function deleteCodeRepository(documentId: string): Promise<void> {
  await rawJson(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}`, { method: 'DELETE' })
}
...
export async function syncCodeRepository(documentId: string): Promise<CodeFileSyncResult> {
  return rawJson<CodeFileSyncResult>(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}/sync`, { method: 'POST' })
}
...
export async function queryCodeSymbols(documentId: string, symbol: string, opts?: { kind?: string; limit?: number }): Promise<CodeSymbolNode[]> {
  const params = new URLSearchParams({ symbol, limit: String(opts?.limit ?? 10) })
  if (opts?.kind) params.set('kind', opts.kind)
  return rawJson<CodeSymbolNode[]>(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}/query?${params}`)
}

export async function listCodeFiles(documentId: string): Promise<CodeFile[]> {
  return rawJson<CodeFile[]>(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}/files`)
}

export async function listAllSymbols(documentId: string, opts?: { kind?: string; limit?: number }): Promise<CodeSymbolNode[]> {
...
export async function getCodeCallers(documentId: string, symbol: string): Promise<CodeSymbolRef[]> {
  const body = await rawJson<{ callers?: CodeSymbolRef[] }>(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}/callers?${new URLSearchParams({ symbol })}`)
  return body.callers ?? []
}

export async function getCodeCallees(documentId: string, symbol: string): Promise<CodeSymbolRef[]> {
  const body = await rawJson<{ callees?: CodeSymbolRef[] }>(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}/callees?${new URLSearchParams({ symbol })}`)
  return body.callees ?? []
}

export async function getCodeImpact(documentId: string, symbol: string, depth = 2): Promise<CodeSymbolRef[]> {
  const params = new URLSearchParams({ symbol, depth: String(depth) })
  const body = await rawJson<{ affected?: CodeSymbolRef[] }>(`/admin/rag/code/repositories/${encodeURIComponent(documentId)}/impact?${params}`)
...
export async function updateL3CacheConfig(config: Partial<L3CacheConfig>): Promise<ApiResponse<L3CacheConfig>> {
  return fetchJson<L3CacheConfig>('/admin/cache/l3/config', { method: 'PUT', body: JSON.stringify(config) })
}

export async function listL3Entries(params: { page?: number; pageSize?: number; mode?: string; userId?: string; sortBy?: string }): Promise<ApiResponse<L3EntriesData>> {
  const qs = new URLSearchParams()
  if (params.page) qs.set('page', String(params.page))
  if (params.pageSize) qs.set('page_size', String(params.pageSize))
...
export async function updateL3EntryMode(pointId: string, mode: 'auto' | 'manual', ttlHours?: number): Promise<ApiResponse<L3CacheEntry>> {
  return fetchJson<L3CacheEntry>(`/admin/cache/l3/entries/${encodeURIComponent(pointId)}/mode`, {
    method: 'PUT',
    body: JSON.stringify({ mode, ttl_hours: ttlHours }),
  })
}

export async function deleteL3Entry(pointId: string): Promise<ApiResponse<{ deleted: boolean }>> {
  return fetchJson<{ deleted: boolean }>(`/admin/cache/l3/entries/${encodeURIComponent(pointId)}`, { method: 'DELETE' })
}

export async function triggerL3Cleanup(): Promise<ApiResponse<{ deleted: number }>> {
  return fetchJson<{ deleted: number }>('/admin/cache/l3/cleanup', { method: 'POST' })
}

async function errorText(res: Response, fallback: string): Promise<string> {
  try {
    const body = await res.json()
    return body?.error?.code || body?.error?.message || body?.detail?.error?.message || body?.detail || `${fallback}: HTTP ${res.status}`
  } catch {
