...
/**
 * Abandon an in-progress draft so a stopped request stops its GPU work too.
 * Without this the backend keeps generating for a client that already left, and
 * the orphan starves the draft the UI is still polling.
 */
export async function cancelDraft(draftId: string): Promise<{ draft_id: string; cancelled: boolean }> { return rawJson(`/admin/draft/${encodeURIComponent(draftId)}/cancel`, { method: 'POST' }) }
