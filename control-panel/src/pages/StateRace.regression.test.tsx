...
describe('latest-request state guards', () => {
  ...
  it('does not let an older L3 response overwrite a newer filter result', async () => {
    type Entry = { id: string; promptPreview: string; model: string; userId: string; createdAt: number; expiresAt: number | null; mode: 'auto' | 'manual'; hitCount: number; tokenCount: number }
    type Page = { data: { items: Entry[]; pagination: { total: number } } }
  ...
  it('keeps the newest trace detail when responses complete out of order', async () => {
    ...
    await user.click(await screen.findByText('trace-a'))
    await user.click(await screen.findByText('trace-b'))
    ...
    const heading = await screen.findByRole('heading', { name: '全链路追踪' })
    const panel = heading.parentElement?.parentElement
    if (!panel) throw new Error('trace detail panel not found')
    expect(within(panel).getByText('trace-b')).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(within(panel).queryByText('trace-a')).not.toBeInTheDocument()
      expect(within(panel).getByText('trace-b')).toBeInTheDocument()
    })
  })
})
