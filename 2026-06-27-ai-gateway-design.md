...
| 命中层 | 回填 L1 | 回填 L2 | 回填 L3 | 说明 |
|--------|--------|--------|--------|------|
| L2 命中 | ✅ | — | — | 热点提升到进程内 |
| L3 命中 | ✅ | ❌ | — | L3 是语义近似匹配，不能用当前精确 key 写 L2 |
| MISS | ✅ | ✅ | ⚠️ 有条件 | 仅 token_count ≥ 100 时写 L3（节省 embedding 计算） |

#### Capacity Protection（容量保护）

- **L1**: LRU 淘汰（maxsize=1000）+ 单条 ≤ 100KB，超大响应不进 L1
- **L2**: Redis `maxmemory 256mb` + `allkeys-lru` 策略 + TTL 3600s + 单条 ≤ 500KB
- **L3**: payload TTL 过期 + 每小时定时清理过期向量（`cleanup_expired_l3`）
- 各级保护互相独立，不会出现低级缓存"撑爆"高级缓存的情况

#### L3 Lifecycle Management（生命周期管理）

每条 L3 缓存条目支持两种管理模式：
- **auto**（默认）：按全局 TTL 自动过期，定时清理任务移除过期条目
- **manual**：不自动过期，仅用户通过控制面板手动删除（适用于知识库/常用问答等重要条目）

控制面板提供 Cache Management 页面：
- 全局配置：清理间隔（分钟）、默认 TTL（小时）、默认管理模式
- 条目列表：按模式/用户/命中次数筛选，支持切换 auto↔manual、手动删除
- 一键清理：立即触发过期条目清理

#### Semantic Cache Reranking（v2 优化）

通过 Retrieve + Rerank 两阶段查询优化语义缓存命中质量：
1. **粗检索**：Qdrant 向量召回 top-K=5 候选（threshold=0.90，比当前松）
2. **精排**：CrossEncoder 对候选做 cross-attention 打分
3. **阈值判断**：rerank_score > 0.85 才命中，否则直接调 LLM

收益：减少"差不多但不对"的缓存命中 → 避免用户重新提问浪费 token。
