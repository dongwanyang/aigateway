...
def _validate_integrations(config: dict[str, Any]) -> list[dict[str, str]]:
    issues: list[dict[str, str]] = []
    for source_kind, source, config_class, env_name in _INTEGRATION_SPECS:
        ...
        if not isinstance(raw, dict):
            ...
            continue
        effective = config_module._apply_env_overrides_for_config(
            env_name,
            raw,
        )
        issues.extend(
...
def _validate_generation(config: dict[str, Any]) -> list[dict[str, str]]:
    ...
    try:
        parsed = GenerationOptimizationConfig.load_from_dict(raw)
        for message in parsed.validate():
            issues.append(_error("generation_optimization", str(message)))
    except Exception as exc:
        issues.append(_error("generation_optimization", str(exc)))
    return issues
...
def validate_component_config_strict(
    ...
    return [*_validate_integrations(config), *_validate_generation(config)]
