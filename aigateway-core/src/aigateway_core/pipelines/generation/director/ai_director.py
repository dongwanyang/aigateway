...
- 超时处理（默认 30 秒），超时或失败时降级到原始 prompt
...
6. 无论用户使用何种语言，视觉描述和四个部分的标题都必须使用自然、准确的英文，以兼容 SDXL 等英文文本编码器
7. 如果用户要求画面中出现特定文字，只将该文字原样保留在英文双引号内，不要翻译该文字
"""
...
5. 无论用户使用何种语言，视觉描述和四个部分的标题都必须使用自然、准确的英文，以兼容 SDXL 等英文文本编码器
6. 如果用户要求画面中出现特定文字，只将该文字原样保留在英文双引号内，不要翻译该文字
"""
