...
async def update_gpu_config(
    ...
    try:
        ...
        # Host inventory is generated by quickstart/render-gpu-topology.py and
        # is intentionally not writable through the policy endpoint. Preserve
        # it verbatim so changing a hot GPU setting cannot destroy restart-only
        # worker/device mappings.
        for key in _GENERATED_TOPOLOGY_FIELDS:
            if key in current:
