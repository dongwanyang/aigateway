...
- **debug_mode**(2026-07-05 起废弃单开关语义) — `debug_mode` 现只用于向后兼容;`_is_debug_mode()` 已删,不再强制 DEBUG 日志级别,不再控制 5xx detail 回显。`AI_GATEWAY_ENV=production` 仍强制 `debug_mode=False` + `log_level≥INFO` 作为生产安全网(config.py 中)。PR2 将用 5 维度 debug 开关(前端/入口/cache/bridge/插件层)替换整个语义。
